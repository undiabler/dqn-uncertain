...
class ReplayBuffer:
    ...
    def __init__(self, size=100000, input_shape=(84, 84), use_per=True):
        """
        Arguments:
        ...
        self.nstates = np.empty((self.size,)+self.input_shape, dtype=np.float32)
    ...
    def add_experience(self, state, action, reward, terminal, next_state, clip_reward=False):
        ...
        if state.shape != self.input_shape:
            raise ValueError('Dimension of frame is wrong!', state.shape, self.input_shape)
        ...
        self.nstates[self.current, ...] = next_state
    ...
    def get_minibatch(self, batch_size=32, priority_scale=0.7):
        ...
        for idx in indices:
            ...
            new_states.append(self.nstates[idx, ...])
        ...
        if self.use_per:
            ...
            importance = sample_probabilities[indices]
            importance = importance / importance.max()

            return (self.states[indices], self.actions[indices], self.rewards[indices], self.nstates[indices], self.terminal_flags[indices]), importance, indices
            # return (states, self.actions[indices], self.rewards[indices], new_states, self.terminal_flags[indices]), importance, indices
        else:
            return self.states[indices], self.actions[indices], self.rewards[indices], self.nstates[indices], self.terminal_flags[indices]
    ...
    def save(self, folder_name):
        ...
        np.save(folder_name + '/nstates.npy', self.nstates)
    ...
    def load(self, folder_name):
        ...
        self.nstates = np.load(folder_name + '/nstates.npy')
